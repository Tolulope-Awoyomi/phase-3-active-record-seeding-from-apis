...
puts "Seeding spells..."
# these are the spells we want to add tot he database
spealls = ["acid-arrow", "animal-messenger", "calm-emotions", "charm-person"]

# iterate over each spell
spealls.each do |spell|
    # make a request to the endpoint for the indivisual spell:
    response = RestClient.get "https://www.dnd5eapi.co/api/spells/#{spell}"

    # the response will come back as a JSON-formatted string.
    # use JSON.parse to convert this string to a Ruby has:
    spell_has = JSON.parse(response)
